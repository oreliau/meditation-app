...
- The kit path is `sandbox/kits/Codex-sonnet` (see its `spec.yaml`) — not `./kits/Codex-sonnet`, an old path referenced in `Docker-sbx.md` that no longer exists in this repo.
